...
EditStat *getEditStats(const char *subject, const char *base) {
  ...
  hashTable *baseIndices = NULL;

  int i, baseLen = strlen(base);

  unsigned int hashSize = baseLen + 'a' - 'A'; 
  // Note 'a' - 'A' to account for collisions that will result from 
  // alphabetic characters wrapping over the small hashTable size

  baseIndices = inithashTableWithSize(baseIndices, hashSize);
  ...
  int nValueFrees = destroyhashTable(baseIndices);
...
int main() {
  ...
  char *w = "emmanlue\0", *base[] = {
    "space\0", "doekee\0", "emmanuel\0", "talons\0", "hashTable\0"
  };
